...
# 	$(HELM_CHARTS_PATH)/$(HELM_CHART_NAME)/values.yaml

TMP_FOLDER="$(PWD)/tmp"

copy-cdrs-from-helm-chart: helm download-helm-chart ## Generates the CRD definitions from the existing helm chart
	@cp $(HELM_CHARTS_PATH)/$(HELM_CHART_NAME)/crds/crd.yaml \
		$(PWD)/config/crd/bases/vault.banzaicloud.com_crds.yaml \
		&& cd $(PWD)/config/crd/ \
		&& $(KUSTOMIZE) edit add resource bases/vault.banzaicloud.com_crds.yaml

helm-chart: helm download-helm-chart copy-cdrs-from-helm-chart ## Downloads and generate the complete helm chart used by the operator
